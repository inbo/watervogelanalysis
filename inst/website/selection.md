...
Keeping so-called structural zero's would flatten the trend (e.g. a site in which the species is always absent).
...
We started by a minimal set of rules and added more rules when we noticed that the models were not stable.
...
We calculate the oldest and most recent winter for which the species was observed at least 5 times in the imputation region.
...
1. Calculate the number of sites where the species was observed for every month and winter.
  ...
  This implies that within the relevant time period, we keep month for which in at least half of the winter the species was observed in at least one site.
...
1. When the date still spans multiple months, remove sites only observed during a single month.
...
1. Some location exhibit strong changes over time.
  We calculate the geometric mean per year for every site and select the five largest values.
  We retain the site when the ratio between the largest and the smallest value of these selected means is less than 10.
  This excluded sites with extreme changes from the imputation model, but not from the totals.
