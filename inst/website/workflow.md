...
We define only two imputation models per species: one at the Belgian level and a second for the Flemish region.
The analyses for regions which are a subset of Flanders reuse the imputations from the model for the Flemish region.
When the analysis contains regions outside of Flanders, we use the imputations from the Belgian model.
...
These [rules](selection.html) split the data into two datasets: the relevant dataset and the rare dataset.
...
Suppose that a species is only observed in Wallonia.
Then we will not analyse the species in Flanders.
