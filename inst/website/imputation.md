...
$$Y_{swm} \sim \mathcal{Binom}(\pi_{swm})\mathcal{TruncNegBin}(\mu_{swm}, n)$${#eq-hurdle}
...
The linear predictor $\zeta_{swm}$ depends on four terms (@eq-zeta):
...
1. $a_w$ is the relative effect of winter $w$ (@eq-presence-winter).
   ...
   $\Delta\Delta_{a_w} = \Delta_{a_w} - \Delta_{a_{w-1}} = (a_{w + 1} - a_{w}) - (a_{w} - a_{w - 1}) = a_{w + 1} - 2 a_{w} + a_{w - 1}$
   The second order difference follows a zero mean Gaussian distribution with variance $\sigma^2_{wp}$.

$$\zeta{swm} = \alpha_0 + \alpha_m + a_s + a_w$${#eq-zeta}

$$a_s \sim \mathcal{N}(0, \sigma^2_{sp})$${#eq-presence-site}

$$a_{w + 1} - 2 a_{w} + a_{w - 1} = \Delta\Delta_{a_w} \sim \mathcal{N}(0, \sigma^2_{wp})$${#eq-presence-winter}
