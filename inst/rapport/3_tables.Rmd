...
results %>%
  filter(ModelType == "imputed trend: Total ~ Year + Month", Duration == 12, 
         Parameter == "cYear") %>%
  mutate(Class = trend_class(LCL, UCL, log(0.75) / Duration)) %>%
  mutate_at(c("Estimate", "LCL", "UCL"), change) %>%
  transmute(
    Area = LocationGroup, Species = scientific_name, 
    Estimate, Class,
    Change = sprintf("%+.0f%% (%+.0f%%; %+.0f%%)", Estimate, LCL, 
                     ifelse(UCL > 1e6, Inf, UCL))
  ) %>%
...
results %>%
  ...
  transmute(
    ...
    Estimate,
    Change = sprintf("%+.0f%% (%+.0f%%; %+.0f%%)", change(Estimate), 
                     change(LCL), ifelse(UCL > 20, Inf, change(UCL)))
  ) %>%
  arrange(Area, Class, desc(Estimate)) %>%
  select(-Estimate) %>%
  group_by(Area) %>%
