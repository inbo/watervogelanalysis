...
sprintf("%s/manifest", project) %>%
  ...
  filter(LastModified >= as.POSIXct("2019-06-20")) %>%
  mutate(
  ...
  unnest(cols = manifest) -> aws_manifest
...
get_species <- function(x) {
  ...
  if (is.null(y)) {
    ...
    store_model(y, base = local_base, project = project)
  }
  if (is.null(y)) {
  ...
  y %>%
    ...
    select(.data$SpeciesGroupID, .data$LocationGroupID)
}
aws_manifest %>%
  filter(is.na(Parent)) %>%
...
local_copy <- function(hash, base, local_base, project) {
  fs::path(local_base, project, stringr::str_trunc(hash, 4, ellipsis = "")) %>%
    list.files(recursive = TRUE) %>%
    stringr::str_subset(hash) -> available
  if (length(available)) {
    if (length(available) > 1) {
      stop("duplicates ", hash)
    }
    fs::path(project, stringr::str_trunc(hash, 4, ellipsis = ""), available) %>%
      get_bucket(bucket = base) -> available_aws
    if (length(available_aws)) {
...
get_monthly_total <- function(x, hash) {
  if (is.null(x@Join)) {
    x@RawImputed@Data %>%
      filter(!.data$Missing) %>%
      mutate(
        Parameter = sprintf("observed total: %i-%s", .data$Year, .data$Month)
      ) %>%
      group_by(.data$Parameter) %>%
      summarise(Estimate = sum(.data$Count)) -> mt
  } else {
    x@RawImputed@Data %>%
    ...
    bind_rows(
      tibble(
        ...
        p05 = apply(x@AggregatedImputed@Imputation, 1, quantile, 0.05,
                  na.rm = TRUE),
        p20 = apply(x@AggregatedImputed@Imputation, 1, quantile, 0.20,
                  na.rm = TRUE),
        p35 = apply(x@AggregatedImputed@Imputation, 1, quantile, 0.35,
                  na.rm = TRUE),
        p65 = apply(x@AggregatedImputed@Imputation, 1, quantile, 0.65,
                  na.rm = TRUE),
        p80 = apply(x@AggregatedImputed@Imputation, 1, quantile, 0.80,
                  na.rm = TRUE),
...
extract_results_mi <- function(x, hash) {
  x@AnalysisMetadata %>%
  select(
    .data$SpeciesGroupID, .data$LocationGroupID, .data$ModelType,
    .data$Duration, .data$FileFingerprint, .data$StatusFingerprint, .data$Status
  ) %>%
...
extract_results_ag <- function(x, hash, aws_manifest) {
  x@AnalysisMetadata %>%
    select(
      .data$SpeciesGroupID, .data$LocationGroupID, .data$ModelType,
      .data$Duration, .data$FileFingerprint, .data$StatusFingerprint,
      .data$Status
    ) %>%
    inner_join(x@AnalysisRelation, by = c("FileFingerprint" = "Analysis")) ->
    result
  ...
  switch(
    ...
    stop(aws_manifest, x@AnalysisMetadata$ModelType, " ", hash)
  ) %>%
...
extract_results_inla <- function(x, hash) {
  ...
  if (!is.null(x@Model$summary.random$LocationID)) {
    x@Model$summary.random$LocationID %>%
      transmute(Parameter = paste("location:", .data$ID), Estimate = mean,
  ...
  if (!is.null(x@RawImputed)) {
    ...
    x@RawImputed@Data %>%
      filter(.data$Missing) %>%
      select(.data$LocationID, .data$Year, .data$Month) %>%
      dplyr::bind_cols(
        data.frame(
          Estimate = apply(x@RawImputed@Imputation, 1, median, na.rm = TRUE),
          LCL = apply(
            x@RawImputed@Imputation, 1, quantile, 0.025, na.rm = TRUE
          ),
          UCL = apply(x@RawImputed@Imputation, 1, quantile, 0.975, na.rm = TRUE)
        )
      ) -> imputations
    imputations %>%
      group_by(.data$LocationID) %>%
      summarise(n = max(.data$Estimate)) %>%
      dplyr::slice_maxtop_n(.data$n, n = 10) %>%
      bind_rows(
        x@RawImputed@Data %>%
          filter(is.na(.data$Count)) %>%
          count(.data$LocationID) %>%
          dplyr::slice_max(.data$n, n = 10)
      ) -> selected
    x@RawImputed@Data %>%
      filter(!.data$Missing) %>%
      select(
        .data$LocationID, .data$Year, .data$Month, Estimate = .data$Count
      ) %>%
      bind_rows(imputations) %>%
      semi_join(selected, by = "LocationID") %>%
      transmute(
        Parameter = sprintf(
          "imputation: %s:%s:%s", .data$LocationID, .data$Year, .data$Month
        ),
        .data$Estimate, .data$LCL, .data$UCL, FileFingerprint = hash
      ) %>%
...
store_results <- function(hash, base, project) {
  ...
  if (file.exists(target)) {
    ...
    readRDS(target) %>%
      distinct(.data$StatusFingerprint) %>%
      pull(.data$StatusFingerprint) -> status_fingerprint
