...
extract_results <- function(hash, base, project, relevant) {
  ...
  if (inherits(x, "n2kAggregate")) {
    ...
    if (x@AnalysisMetadata$Status == "converged") {
      switch(
        ...
        stop(x@AnalysisMetadata$ModelType, " ", hash)
      ) %>%
  ...
  if (inherits(x, "n2kInla")) {
    x@AnalysisMetadata %>%
      ...
      result
    if (x@AnalysisMetadata$Status == "converged") { 
        estimates <- data.frame(Parameter = character(0))
        ...
        result %>%
          left_join(estimates, by = "FileFingerprint") -> result
    }
    return(result)
  }
...
local_copy <- function(hash, base, local_base, project) {
  ...
  if (length(available)) {
    ...
    if (length(available.aws)) {
      ...
      return(file.path(project, str_trunc(hash, 4, ellipsis = ""), available))
    }
  }
  message(hash, " update")
  read_model(hash, base, project) %>%
    store_model(local_base, project, overwrite = TRUE)
}
```
...
sprintf("%s/manifest", project) %>%
  ...
  filter(LastModified >= max(LastModified) - 2 * 3600) %>%
  mutate(
    manifest = str_extract(Key, "([[:xdigit:]]{40})") %>%
      map(read_manifest, base = base, project = project)
  ) -> manifests
sapply(manifests$manifest, store_manifest, base = local_base, 
       project = project) -> stored_manifest
manifests %>%
  mutate(manifest = map(manifest, slot, "Manifest")) %>%
  unnest() -> relevant
relevant %>%
  distinct(Fingerprint) %>%
  arrange(Fingerprint) %>%
  mutate(stored = map_chr(Fingerprint, local_copy, base = base,
                          local_base = local_base, project = project)
  ) -> junk

relevant %>%
  arrange(Fingerprint) %>%
  mutate(results = map(Fingerprint, extract_results, base = local_base,
                       project = project, relevant = relevant)) %>%
  unnest() %>%
  inner_join(location_group, by = "LocationGroupID") %>%
  inner_join(species_list, by = "SpeciesGroupID") -> results
