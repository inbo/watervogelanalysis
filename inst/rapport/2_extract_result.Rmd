...
get_manifest <- function(hash, local) {
  ...
  store_manifest(x, base = aws_base, project = project)
...
aws_manifest %>%
  arrange(desc(Level), desc(LastModified)) %>%
  group_by(hash) %>%
  slice(1) %>%
  ungroup() %>%
  mutate(meta = map2(Fingerprint, local, ~get_species(x = .x, local = .y)))  %>%
  unnest() -> manifest_species_location
manifest_species_location %>%
  filter(is.na(SpeciesGroupID)) %>%
  select(Key, hash, local, LastModified) %>%
...
relevant %>%
  ...
  arrange(desc(LastModified)) %>%
  group_by(SpeciesGroup, LocationGroup) %>%
  slice(1) %>%
  ungroup() %>%
...
required_results[required_results %in% prepared_results] %>%
  lapply(readRDS) %>%
  bind_rows() %>%
  inner_join(relevant, by = c("FileFingerprint" = "Fingerprint")) %>%
  saveRDS("results.Rds")
```
