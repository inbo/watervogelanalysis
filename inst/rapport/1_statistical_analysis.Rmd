...
We aggregate this augmented data into a monthly population total $T_{wm}$ by summation of the counts of every site $s$ for each combination of winter $w$ and month $m$.
...
We assume that the monthly population total  follows a negative binomial distribution with mean $\mu_{wm}$ and overdispersion parameter $n$.
...
$$I_{ir} = \frac{S_i}{S_r}$$
We can substitute $S_w$ with the formula we gave above, resulting in
...
$e^{W\beta_w}$ estimates the total linear change over a period $W$.
