...
remove_duplicate_observation <- function(observation){
  check_dataframe_variable(
    df = observation, 
    variable = c("ObservationID", "DatasourceID", "LocationID", "Year", "fMonth", "Complete", "Count"),
    name = "observation"
  )
  ...
  observation.to.clean <- observation[duplicate, ]
  duplicate.id <- observation.to.clean[, c("ObservationID", "DatasourceID")]

  # remove uncomplete replicate if complete replicate exist
  max.complete <- aggregate(
    observation.to.clean[, "Complete", drop = FALSE],
    observation.to.clean[, c("LocationID", "Year", "fMonth")],
    FUN = max
  )
